...
static inline bool __is_full(min_heap_t *h)
{
	return h->size == h->capacity;
}

static inline bool __is_empty(min_heap_t *h)
{
	return h->size == 0;
}
...
static inline void __heapify_up(void **heap, cmp_fn heap_cmp, int idx)
{
	...
	// loop until parent is smaller than current node
	pidx = PARENT_IDX(idx);
...
static inline void __heapify_down(void **heap, cmp_fn heap_cmp, int idx, int size)
{
	...
	// loop until curent node (root) is smaller than both children
	while (1) {
		lidx = LEFT_IDX(idx);
		ridx = RIGHT_IDX(idx);
		...
		// compute smallest node between node and its children
		tmp = idx;
		...
		// stop if node is already smaller than its children
		if (tmp == idx)
			break;
...
/**
 * Create a min-heap entry.
 *
 * @size	: Min-Heap maximum size.
						...
						cmp_fn _cmp, print_fn _print)
{
	...
	// min-heap object
	entry = (min_heap_t *)malloc(sizeof(min_heap_t));
	if (!entry)
		goto error;
...
/**
 * Destroy (free memory) for a min-heap.
 */
...
/**
 * Add new entry to min-heap.
 *
 * @h	: Min-Heap object.
 * @data: Data to be added to min-heap.
 *
...
/**
 * Peek top element from min-heap (smallest).
 *
 * @h	: Min-Heap object.
 *
